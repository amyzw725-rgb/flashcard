...
用浏览器直接打开 Flashcard.py.py（或重命名后的 Flashcard.html）
...
按主题生成单词卡，Body: {"topic": "旅行", "count": 5}
...
词库类别包括：travel、food、shopping、business、daily_life 等。call_deepseek_api 目前为本地词库模拟，按主题关键词匹配类别后随机抽词，并非真实调用 DeepSeek。
...
例句（HTML，含 <span class="highlight"> 高亮）
